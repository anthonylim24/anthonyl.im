  ...
  return (
    ...
    >
      ...
      <motion.div variants={fadeUp} className="pt-2 pb-8 md:pb-20">
        <p className="text-sm text-zinc-400 font-medium tracking-wide md:hidden">
          {getGreeting()}
        </p>
        <h1 className="font-display text-[clamp(1.75rem,6vw,2rem)] md:text-[clamp(2.75rem,8vw,4.5rem)] font-extrabold text-zinc-900 tracking-[-0.03em] leading-[0.95] mt-1 md:mt-0">
          <span className="md:hidden">Time to breathe</span>
          <span className="hidden md:inline">{getGreeting()}</span>
        </h1>
        <p className="text-sm text-zinc-400 mt-2 md:mt-3 font-medium tracking-wide hidden md:block">
          {isNewUser
      ...
      {isNewUser ? (
        ...
        <>
          ...
          <motion.div variants={fadeUp} className="pb-8 md:hidden">
            ...
            <button
              ...
              className="w-full mt-3 text-sm text-zinc-400 font-medium hover:text-zinc-600 transition-colors py-2"
            >
          ...
          {/* Desktop welcome */}
          <motion.div variants={fadeUp} className="hidden md:block pb-20 border-b border-zinc-100">
            <button
              onClick={() => { haptic('success'); navigate('/breathwork/session?technique=box_breathing') }}
            ...
            <button
              ...
              className="mt-4 text-sm text-zinc-400 font-medium hover:text-zinc-600 transition-colors"
            >
        ...
        <>
          ...
          <motion.div variants={fadeUp} className="flex gap-2 pb-6 md:hidden">
            <div className="flex-1 rounded-xl border border-zinc-200 px-3 py-2.5">
              <span className="font-display text-lg font-bold text-zinc-900 tabular-nums leading-none">
                Lv {level}
              </span>
              <span className="block text-[10px] text-zinc-400 font-medium tracking-wide uppercase mt-1">
                Level
              </span>
            </div>
            <div
            ...
            >
              <span
                className="font-display text-lg font-bold text-zinc-900 tabular-nums leading-none"
          ...
          <motion.div variants={fadeUp} className="hidden md:flex items-baseline gap-12 pb-20 border-b border-zinc-100">
            <div>
              <span className="font-display text-4xl font-extrabold text-zinc-900 tabular-nums leading-none">
                {getLevelTitle(level)}
              </span>
              <span className="block text-[11px] text-zinc-300 font-semibold tracking-[0.08em] uppercase mt-1.5">
                Level {level}
              </span>
            </div>
            <div>
          ...
          <motion.div variants={fadeUp} className="py-5 sm:py-6">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[11px] text-zinc-300 font-semibold tracking-[0.08em] uppercase">
      ...
      <motion.div variants={fadeUp} className="pt-8 md:pt-16" id="techniques-section">
        <h2 className="font-display text-xl md:text-3xl font-bold text-zinc-900 tracking-tight mb-5 md:mb-10">
          Techniques
        </h2>
            ...
            {techniques.map((id) => {
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  transition={spring}
                  className="rounded-2xl p-4 text-left border border-zinc-200 bg-zinc-50 flex-shrink-0"
                ...
                >
                  ...
                  <h3 className="font-display text-sm font-bold text-zinc-900 leading-tight">
                    {p.name}
                  </h3>
                  <div className="flex items-center gap-1 mt-2 flex-wrap">
                    {p.phases.map((phase, i) => (
                      <span key={i} className="flex items-center gap-0.5">
                        <span className="text-[10px] font-mono text-zinc-400">{phase.duration}s</span>
                        {i < p.phases.length - 1 && (
            ...
            {restTechniques.map((id) => {
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.99 }}
                  transition={spring}
                  className="w-full flex items-center gap-4 p-5 rounded-2xl text-left group hover:bg-zinc-50 transition-colors duration-200"
                  onClick={() => { haptic('light'); navigate(`/breathwork/session?technique=${id}`) }}
                >
                  <div
                  ...
                  <div className="flex-1 min-w-0">
                    <h3 className="font-display text-base font-bold text-zinc-900 leading-tight">
                      {protocol.name}
                    </h3>
                  ...
                  <ArrowRight className="h-4 w-4 text-zinc-300 shrink-0 group-hover:text-zinc-400 group-hover:translate-x-0.5 transition-all duration-200" />
                </motion.button>
              )
            })}
          </div>
      ...
      {sessions.length > 0 && (
        <motion.div variants={fadeUp} className="pt-16 sm:pt-20">
          <div className="flex items-center justify-between mb-6">
            <h2 className="font-display text-2xl sm:text-3xl font-bold text-zinc-900 tracking-tight">Recent</h2>
            <button
              onClick={() => { haptic('selection'); navigate('/breathwork/progress') }}
              className="flex items-center gap-1 text-sm font-medium text-zinc-300 hover:text-zinc-600 transition-colors"
            >
              All
            ...
            {sessions.slice(0, 3).map((session) => {
              ...
              return (
                <motion.button
                  ...
                  whileTap={{ scale: 0.99 }}
                  transition={spring}
                  className="w-full flex items-center gap-4 p-4 sm:p-5 rounded-2xl text-left group hover:bg-zinc-50 transition-colors duration-200"
                  onClick={() => { haptic('selection'); navigate('/breathwork/progress') }}
                >
                  <div
                    className="h-10 w-10 rounded-xl flex items-center justify-center shrink-0"
                  ...
                  <ChevronRight className="h-4 w-4 text-zinc-200 shrink-0 group-hover:text-zinc-400 transition-colors" />
                </motion.button>
              )
            })}
          </div>
