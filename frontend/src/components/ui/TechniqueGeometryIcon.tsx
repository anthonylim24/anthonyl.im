...
export function TechniqueGeometryIcon({ techniqueId, className, size = 16 }: TechniqueGeometryIconProps) {
  ...
  switch (geometry) {
    case 'grid':
      return (
        <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} fill="none" stroke="currentColor" strokeWidth="1" className={className}>
          {[0, 1, 2, 3, 4].map(i => {
            const pos = (i / 4) * size
