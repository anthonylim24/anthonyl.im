  ...
  if (sessions.length === 0) {
    return (
      <div className="py-10 text-center surface-well rounded-[18px] text-sm">
        <p className="text-zinc-400">Your session history will appear here after your first practice.</p>
        <button
          onClick={() => navigate('/breathwork/session?technique=box_breathing')}
          className="mt-3 text-sm font-medium text-zinc-500 hover:text-zinc-700 transition-colors"
        >
  ...
  return (
    <div className="space-y-2">
      {sessions.map((session) => (
        <div
          key={session.id}
          className="flex items-center gap-3 p-3.5 rounded-2xl surface-well hover:bg-zinc-50 transition-colors duration-200"
        >
          {/* Technique icon */}
          <div
          ...
          <div className="flex-1 min-w-0">
            <div className="font-semibold text-sm text-zinc-900 leading-tight truncate">
          ...
          {session.maxHoldTime > 0 && (
            <div className="shrink-0 text-right pl-2 border-l border-zinc-200">
              <div className="flex items-center gap-1 font-display text-sm font-bold text-zinc-900 tabular-nums">
