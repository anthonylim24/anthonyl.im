...
Code lives in `frontend/src/breathflow/` (pages, engine, protocols, gamify, motion, platform). Session state is `useSessionEngine` — not a Zustand `sessionStore`.
...
2. For Clerk-gated preview routes (`/korea`, `/trips`), mint a session with `bun scripts/clerk-agent-login.ts --pr <n> --path /korea` or `--path /trips` (use the matching path). The helper re-execs from `origin/main` before sending secrets. Cursor cloud `gh` tokens have no push — `CLERK_SECRET_KEY` is enough (screenshot-user default); after production deploys the installation-token check, `gh auth token` works too. Open the printed Clerk URL in Chrome MCP. Dedicated screenshot identity, not a personal production login — do not sign in to production `/korea` or `/trips`. Public routes only need `?hidePreviewChrome=1`.
