...
Plans for B2C must be created as **User Plans** (Dashboard → Billing → Plans → User Plans tab). A `pro` plan registered as an Organization Plan is a separate entity and won't appear in `<PricingTable />`. Plan type isn't changeable, recreate if misplaced.
...
export function ExportButton() {
	...
	if (!has?.({ feature: 'export' })) {
		return (
			<button disabled title="Export feature required">
				Export
			</button>
		)
	}
