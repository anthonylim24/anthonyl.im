...
- **Two setup paths.** Dashboard for interactive UI, or `clerk api` for scripted setup: `clerk api -X POST /enterprise_connections` (create), `clerk api -X PATCH /enterprise_connections/{id}` (update), `clerk api -X DELETE /enterprise_connections/{id}` (remove). Pass the IdP metadata or client credentials in the request body; Clerk returns the ACS URL + Entity ID in the response. Create / update endpoints require a plan with the SAML feature enabled. The legacy `/saml_connections` endpoint is deprecated, use `/enterprise_connections` instead.
